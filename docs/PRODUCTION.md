# Core Foundation V2 — Tahap B: Data Integrity & Audit Trail (2026-09-16)
...
- Append-only: grants hanya SELECT authenticated dengan RLS active super_admin/kepala_sekolah; public/anon/authenticated/service_role tidak diberi direct writes. Guard menolak UPDATE/DELETE/TRUNCATE dan menstempel actor/time saat INSERT. Owner/superuser DDL tetap di luar jaminan immutability aplikasi.
...
- Metadata hanya changed_fields dan old/new status/role. Tidak ada salinan profil, nilai kontak, isi feedback, password/token/cookie/key/session/raw error stack. Entity UUID tampil sebagai referensi; actor name dibaca melalui profiles RLS pada waktu render, bukan snapshot.
...
Validation: 63 foundation tests lulus (54 tahap A + 9 baru), mencakup formatter secret filtering, capability, tenant scoping/actor lookup, missing schema/error handling, Activity render per role, SQL static guards/grants/coverage/metadata, dan manifest checksum 001–006. Ini static/unit/mock, bukan SQL runtime atau production RLS validation. Lint/typecheck/build/diff-check dicatat setelah pemeriksaan akhir.
